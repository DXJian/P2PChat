...
if(p[0]==='pencil'){tempArray[i]=['context.beginPath();\n'+'context.moveTo('+point[0]+', '+point[1]+');\n'+'context.lineTo('+point[2]+', '+point[3]+');\n'+this.strokeOrFill(p[2])]}
...
var drawHelper={redraw:function(){tempContext.clearRect(0,0,innerWidth,innerHeight);context.clearRect(0,0,innerWidth,innerHeight);context.fillStyle='white';context.globalAlpha=1;context.fillRect(0,0,innerWidth,innerHeight);var i,point,length=points.length;for(i=0;i<length;i++){point=points[i];if(point&&point.length&&this[point[0]]){this[point[0]](context,point[1],point[2])}}},refreshColorValues(){var strokeStyleText=find('stroke-style'),fillStyleText=find('fill-style');strokeStyle=strokeStyleText.value;fillStyle=fillStyleText.value},getOptions:function(opt){opt=opt||{};return[opt.lineWidth||lineWidth,opt.strokeStyle||strokeStyle,opt.fillStyle||fillStyle,opt.globalAlpha||globalAlpha,opt.globalCompositeOperation||globalCompositeOperation,opt.lineCap||lineCap,opt.lineJoin||lineJoin,opt.font||font]},handleOptions:function(context,opt,isNoFillStroke){opt=opt||this.getOptions();context.globalAlpha=opt[3];context.globalCompositeOperation=opt[4];context.lineCap=opt[5];context.lineJoin=opt[6];context.lineWidth=opt[0];context.strokeStyle=opt[1];context.fillStyle=opt[2];context.font=opt[7];if(!isNoFillStroke){context.stroke();context.fill()}},line:function(context,point,options){this.refreshColorValues();context.beginPath();context.moveTo(point[0],point[1]);context.lineTo(point[2],point[3]);this.handleOptions(context,options)},pencil:function(context,point,options){context.beginPath();context.moveTo(point[0],point[1]);context.lineTo(point[2],point[3]);this.handleOptions(context,options)},marker:function(context,point,options){context.beginPath();context.moveTo(point[0],point[1]);context.lineTo(point[2],point[3]);this.handleOptions(context,options)},arrow:function(context,point,options){var mx=point[0];var my=point[1];var lx=point[2];var ly=point[3];var arrowSize=arrowHandler.arrowSize;if(arrowSize==10){arrowSize=(options?options[0]:lineWidth)*5}
...
context.drawImage(image,point[1],point[2],point[3],point[4]);pdfHandler.reset_pos(point[1],point[2])},quadratic:function(context,point,options){this.refreshColorValues();context.beginPath();context.moveTo(point[0],point[1]);context.quadraticCurveTo(point[2],point[3],point[4],point[5]);this.handleOptions(context,options)},bezier:function(context,point,options){this.refreshColorValues();context.beginPath();context.moveTo(point[0],point[1]);context.bezierCurveTo(point[2],point[3],point[4],point[5],point[6],point[7]);this.handleOptions(context,options)}};var dragHelper={global:{prevX:0,prevY:0,ismousedown:!1,pointsToMove:'all',startingIndex:0},mousedown:function(e){if(isControlKeyPressed){copy();paste();isControlKeyPressed=!1}
...
if(p[0]==='pencil'){if(dHelper.isPointInPath(x,y,point[0],point[1])){g.pointsToMove='head'}
if(dHelper.isPointInPath(x,y,point[2],point[3])){g.pointsToMove='tail'}}
...
if(p[0]==='pencil'){tempContext.beginPath();tempContext.arc(point[0],point[1],10,Math.PI*2,0,!1);tempContext.arc(point[2],point[3],10,Math.PI*2,0,!1);tempContext.fill()}
...
if(p[0]==='pencil'){points[i]=[p[0],[getPoint(x,prevX,point[0]),getPoint(y,prevY,point[1]),getPoint(x,prevX,point[2]),getPoint(y,prevY,point[3])],p[2]]}
...
if(p[0]==='pencil'){if(g.pointsToMove==='head'||isMoveAllPoints){point[0]=getPoint(x,prevX,point[0]);point[1]=getPoint(y,prevY,point[1])}
if(g.pointsToMove==='tail'||isMoveAllPoints){point[2]=getPoint(x,prevX,point[2]);point[3]=getPoint(y,prevY,point[3])}
points[points.length-1]=[p[0],point,p[2]]}
...
points[points.length-1]=[p[0],point,p[2]]}}};var pencilHandler={ismousedown:!1,prevX:0,prevY:0,mousedown:function(e){var x=e.pageX-canvas.offsetLeft,y=e.pageY-canvas.offsetTop;var t=this;t.prevX=x;t.prevY=y;t.ismousedown=!0;tempContext.lineCap='round';pencilDrawHelper.pencil(tempContext,[t.prevX,t.prevY,x,y]);points[points.length]=['pencil',[t.prevX,t.prevY,x,y],pencilDrawHelper.getOptions(),'start'];t.prevX=x;t.prevY=y},mouseup:function(e){var x=e.pageX-canvas.offsetLeft,y=e.pageY-canvas.offsetTop;var t=this;if(t.ismousedown){tempContext.lineCap='round';pencilDrawHelper.pencil(tempContext,[t.prevX,t.prevY,x,y]);points[points.length]=['pencil',[t.prevX,t.prevY,x,y],pencilDrawHelper.getOptions(),'end'];t.prevX=x;t.prevY=y}
this.ismousedown=!1},mousemove:function(e){var x=e.pageX-canvas.offsetLeft,y=e.pageY-canvas.offsetTop;var t=this;if(t.ismousedown){tempContext.lineCap='round';pencilDrawHelper.pencil(tempContext,[t.prevX,t.prevY,x,y]);points[points.length]=['pencil',[t.prevX,t.prevY,x,y],pencilDrawHelper.getOptions()];t.prevX=x;t.prevY=y}}}
...
window.addEventListener('load',function(){setDefaultSelectedIcon()},!1);(function(){var cache={};var lineCapSelect=find('lineCap-select');var lineJoinSelect=find('lineJoin-select');function getContext(id){var context=find(id).getContext('2d');context.lineWidth=2;context.strokeStyle='#6c96c8';return context}
function bindEvent(context,shape){if(shape==='Pencil'||shape==='Marker'){lineCap=lineJoin='round'}
addEvent(context.canvas,'click',function(){if(textHandler.text.length){textHandler.appendPoints()}
...
var toolBox=find('tool-box');toolBox.style.height=(innerHeight)+'px';function decorateDragLastPath(){var context=getContext('drag-last-path');var image=new Image();image.onload=function(){context.drawImage(image,6,6,28,28);bindEvent(context,'DragLastPath')};image.src=data_uris.dragSingle;addEvent(context.canvas,'click',function(){hideContainers()})}
decorateDragLastPath();if(tools.dragSingle===!0){document.getElementById('drag-last-path').style.display='block'}
function decorateDragAllPaths(){var context=getContext('drag-all-paths');var image=new Image();image.onload=function(){context.drawImage(image,6,6,28,28);bindEvent(context,'DragAllPaths')};image.src=data_uris.dragMultiple;addEvent(context.canvas,'click',function(){hideContainers()})}
decorateDragAllPaths();if(tools.dragMultiple===!0){document.getElementById('drag-all-paths').style.display='block'}
function decorateLine(){var context=getContext('line');var image=new Image();image.onload=function(){context.drawImage(image,6,6,28,28);bindEvent(context,'Line')};image.src=data_uris.line;addEvent(context.canvas,'click',function(){hideContainers()})}
if(tools.line===!0){decorateLine();document.getElementById('line').style.display='block'}
function decorateUndo(){var context=getContext('undo');var image=new Image();image.onload=function(){context.drawImage(image,6,6,28,28);document.querySelector('#undo').onclick=function(){hideContainers();if(points.length){points.length=points.length-1;drawHelper.redraw()}
syncPoints(!0)}};image.src=data_uris.undo}
if(tools.undo===!0){decorateUndo();document.getElementById('undo').style.display='block'}
function decorateArrow(){var context=getContext('arrow');var image=new Image();image.onload=function(){context.drawImage(image,6,6,28,28);bindEvent(context,'Arrow')};image.src=data_uris.arrow}
if(tools.arrow===!0){decorateArrow();document.getElementById('arrow').style.display='block'}
function decoreZoomUp(){var context=getContext('zoom-up');addEvent(context.canvas,'click',function(){zoomHandler.up()});var image=new Image();image.onload=function(){context.drawImage(image,6,6,28,28)};image.src=data_uris.zoom_in}
function decoreZoomDown(){var context=getContext('zoom-down');addEvent(context.canvas,'click',function(){zoomHandler.down()});var image=new Image();image.onload=function(){context.drawImage(image,6,6,28,28)};image.src=data_uris.zoom_out}
...
function decorateEraser(){var context=getContext('eraser-icon');var image=new Image();image.onload=function(){context.drawImage(image,6,6,28,28);bindEvent(context,'Eraser')};image.src=data_uris.eraser;addEvent(context.canvas,'click',function(){hideContainers()});addEvent(context.canvas,'dblclick',function(){hideContainers();var r=confirm("Clear everything?");if(r==!0){points=[];drawHelper.redraw();return}})}
if(tools.eraser===!0){decorateEraser();document.getElementById('eraser-icon').style.display='block'}
function decorateText(){var context=getContext('text-icon');var image=new Image();image.onload=function(){context.drawImage(image,6,6,28,28);bindEvent(context,'Text')};image.src=data_uris.text;addEvent(context.canvas,'click',function(){hideContainers()})}
if(tools.text===!0){decorateText();document.getElementById('text-icon').style.display='block'}
function decorateImage(){var context=getContext('image-icon');var image=new Image();image.onload=function(){context.drawImage(image,6,6,28,28);bindEvent(context,'Image')};image.src=data_uris.image;addEvent(context.canvas,'click',function(){hideContainers()})}
if(tools.image===!0){decorateImage();document.getElementById('image-icon').style.display='block'}
function decoratePDF(){var context=getContext('pdf-icon');var image=new Image();image.onload=function(){context.drawImage(image,6,6,28,28);bindEvent(context,'Pdf')};image.src=data_uris.pdf;addEvent(context.canvas,'click',function(){hideContainers()})}
if(tools.pdf===!0){decoratePDF();document.getElementById('pdf-icon').style.display='block'}
function decorateArc(){var context=getContext('arc');var image=new Image();image.onload=function(){context.drawImage(image,6,6,28,28);bindEvent(context,'Arc')};image.src=data_uris.arc;addEvent(context.canvas,'click',function(){hideContainers()})}
if(tools.arc===!0){decorateArc();document.getElementById('arc').style.display='block'}
function decorateRect(){var context=getContext('rectangle');var image=new Image();image.onload=function(){context.drawImage(image,6,6,28,28);bindEvent(context,'Rectangle')};image.src=data_uris.rectangle;addEvent(context.canvas,'click',function(){hideContainers()})}
if(tools.rectangle===!0){decorateRect();document.getElementById('rectangle').style.display='block'}
function decorateQuadratic(){var context=getContext('quadratic-curve');var image=new Image();image.onload=function(){context.drawImage(image,6,6,28,28);bindEvent(context,'QuadraticCurve')};image.src=data_uris.quadratic;addEvent(context.canvas,'click',function(){hideContainers()})}
if(tools.quadratic===!0){decorateQuadratic();document.getElementById('quadratic-curve').style.display='block'}
function decorateBezier(){var context=getContext('bezier-curve');var image=new Image();image.onload=function(){context.drawImage(image,6,6,28,28);bindEvent(context,'Bezier')};image.src=data_uris.bezier;addEvent(context.canvas,'click',function(){hideContainers()})}
...
function setTemporaryLine(){var arr=["line",[139,261,170,219],[1,"rgba(0,0,0,0)","rgba(0,0,0,0)",1,"source-over","round","round","15px \"Arial\""]];points.push(arr);drawHelper.redraw();setTimeout(function(){setSelection(document.getElementById('line'),'Line')},1000);setTimeout(setDefaultSelectedIcon,2000)}
...
if(event.data.renderStream){setTemporaryLine();return}
...
if(event.data.undo&&points.length){var index=event.data.index;if(event.data.tool){var newArray=[];var length=points.length;var reverse=points.reverse();for(var i=0;i<length;i++){var point=reverse[i];if(point[0]!==event.data.tool){newArray.push(point)}}
points=newArray.reverse();drawHelper.redraw();syncPoints(!0);return}
if(index==='all'){points=[];drawHelper.redraw();syncPoints(!0);return}
...
if(index===-1){if(points.length&&points[points.length-1][0]==='pencil'){var newArray=[];var length=points.length;var reverse=points.reverse();var ended;for(var i=0;i<length;i++){var point=reverse[i];if(point[3]=='start'){ended=!0}else if(ended){newArray.push(point)}}
points=newArray.reverse();drawHelper.redraw();syncPoints(!0);return}
points.length=points.length-1;drawHelper.redraw();syncPoints(!0);return}
...
if(event.data.clearCanvas){points=[];drawHelper.redraw();return}
